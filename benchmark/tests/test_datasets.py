...
def test_type_inhabitation_top25_all_packages_in_phase2_benchmark() -> None:
    ...
    top25_ids = set(
        line.strip() for line in top25_path.read_text(encoding="utf-8").splitlines()
    ...
    phase2_ids = set(
        line.strip() for line in phase2_path.read_text(encoding="utf-8").splitlines()
        if line.strip() and not line.strip().startswith("#")
    )
...
def test_type_inhabitation_top25_format_compliance() -> None:
    ...
    package_lines = [i for i, line in enumerate(content) 
                    if line.strip() and not line.strip().startswith("#")]
...
def test_dataset_format_standardization() -> None:
    ...
    for dataset_file in datasets_dir.glob("*.txt"):
        ...
        assert len(package_lines) == len(set(package_lines)), \
            f"Dataset {dataset_file.name} has duplicate package IDs"
        
        # No trailing whitespace
        has_trailing = any(line != line.rstrip() for line in content)
        assert not has_trailing, \
            f"Dataset {dataset_file.name} has trailing whitespace"
